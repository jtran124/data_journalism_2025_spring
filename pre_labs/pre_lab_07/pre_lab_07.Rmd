...
```{r echo=FALSE}
census_api_key("3915bf31af618991301cb650b9af4e1ffcf7c17d", install=TRUE)
```
...
```{r}
alldata |> mutate(change = ((Population2000-Population2010)/Population2010)*100) |> arrange(desc(change))
```
...
Is the difference between Calvert and Montgomery significant? No. They are very close together.
...
students enrolled by zipcode with a median income of 75,000 or 40,000

md_high_schools_with_acs |>
  filter(median_household_income >= 75000) |> 
  summarize(avg_enrollment = mean(student_enrollment), count = n())

md_high_schools_with_acs |>
  filter(median_household_income <= 40000) |> 
  summarize(avg_enrollment = mean(student_enrollment), count = n())
